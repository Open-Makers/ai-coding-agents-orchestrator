...
func (t *TmuxMultiplexer) CreateSession(id string) error {
	t.sessionID = id
	return t.tmux("new-session", "-d", "-s", id, "-x", "220", "-y", "50")
}

// NewPane creates a new window for the given role. The first call uses the
// initial window; subsequent calls open new windows.
func (t *TmuxMultiplexer) NewPane(role bus.AgentRole) (PaneID, error) {
	var target string
	if t.paneCount == 0 {
		// Use the first window created with the session.
		target = fmt.Sprintf("%s:0", t.sessionID)
		// Rename the window.
		_ = t.tmux("rename-window", "-t", target, string(role))
	} else {
		// Open a new window.
		windowIdx := fmt.Sprintf("%d", t.paneCount)
		if err := t.tmux("new-window", "-t", t.sessionID, "-n", string(role)); err != nil {
			return "", fmt.Errorf("tmux new-window: %w", err)
		}
		target = fmt.Sprintf("%s:%s", t.sessionID, windowIdx)
	}
	t.paneCount++
...
func (t *TmuxMultiplexer) WriteToPane(id PaneID, text string) error {
	...
	return t.tmux("send-keys", "-t", target, safe)
}
...
func (t *TmuxMultiplexer) SendCommand(id PaneID, cmd string) error {
	...
	return t.tmux("send-keys", "-t", target, safe, "Enter")
}
